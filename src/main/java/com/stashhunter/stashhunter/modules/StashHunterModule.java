...
public class StashHunterModule extends Module {
    ...
    private void onTick(TickEvent.Post event) {
        ...
        // Clean up old reported stashes every 10 minutes
    ...
    private void processCluster(List<BlockPos> cluster) {
        ...
        // Check if we've already reported a stash near this location
        ...
        if (isNaturalStructure(cluster, volume, density)) {
            info("Skipped natural structure at " + stashPos.toShortString() +
    ...
    private void reportStash(BlockPos stashPos, List<BlockPos> valuableBlocks, double volume, double density) {
        ...
        String modeInfo = storageOnlyMode.get() ? " (Storage Only)" : " (Full Scan)";
        String logMessage = String.format("Stash found at: %s with %d valuable blocks (density: %.6f, mode: %s)",
            coords, valuableBlocks.size(), density, storageOnlyMode.get() ? "Storage Only" : "Full Scan");
        ...
        if (density >= Config.notificationDensityThreshold) {
            ...
            String title = storageOnlyMode.get() ? "Stash Found! (Storage Only)" : "Stash Found! (Full Scan)";
