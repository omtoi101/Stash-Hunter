...
import meteordevelopment.meteorclient.systems.modules.misc.AutoTool;
import meteordevelopment.meteorclient.systems.modules.player.AutoExp;
...
public class AutoElytraRepair extends Module {
    ...
    private void handleFindingLanding() {
        ...
        BlockPos landingSpot = SafeLandingSpotFinder.findSafeLandingSpot(
            playerPos, landingRadius.get(), mc.world
        );
    ...
    private void handleRepairing() {
        ...
        // Move elytra to offhand for repair
        if (!isElytraInOffhand(elytra)) {
            moveElytraToOffhand(slotIndex);
            info("Moved elytra to offhand for repair (slot " + slotIndex + ")");
        }
    ...
    private void handleResuming() {
        ...
        if (mc.player.isOnGround()) {
            ...
            // After a brief delay, start elytra flight
            new Thread(() -> {
                try {
                    Thread.sleep(500); // Wait for jump
                    mc.options.jumpKey.setPressed(false);
                    
                    Thread.sleep(1000); // Wait to gain some altitude
                    ...
                    Thread.sleep(2000); // Wait for stable flight
